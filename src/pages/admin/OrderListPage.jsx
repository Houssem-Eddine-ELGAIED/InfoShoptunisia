...
const OrderListsPage = () => {
  ...
  const handleRemoveOrder = async (orderId) => {
    if (window.confirm('Are you sure you want to remove this order?')) {
      try {
        await deleteOrder(orderId); // Appel à la mutation pour supprimer la commande
        alert('Order removed successfully'); // Alerte de succès
      } catch (error) {
        alert('Failed to remove order'); // En cas d'échec
      }
    }
  };
      ...
      {isLoading ? (
        ...
        <Table striped hover bordered responsive size='sm'>
          ...
          <tbody>
            {orders?.map(order => (
              <tr key={order._id}>
                <td>{order.user?._id || 'N/A'}</td> {/* Check if user exists */}
                <td>{order.user?.name || 'N/A'}</td> {/* Check if user exists */}
                ...
                <td>
                  ...
                  {userInfo.isAdmin && (
                    <Button
                      ...
                      disabled={deleting} // Désactivez le bouton pendant que la suppression est en cours
                    >
